...
public class MusicPlayer : MonoBehaviour {
	...
	void CreateSongSelectionList () {
		...
		foreach (SongData song in songs) {
			...
			clone.name = string.Format ("{0} - {1}", song.artist, song.songName);
			...
			// Show Song Data
			clone.GetComponent<ActivateSong> ().ShowSongData (song.artist, song.songName, pos, minutes, seconds);
	...
	public void ChangeSong (int pos) {
		...
		songName.text = string.Format ("{0} - {1}", songs [pos].artist, songs [pos].songName);
